...
private static float resizeStartX, resizeStartY;
private static float startX, startY, startWidth, startHeight;
...
float gifX = (config.xPos / 100f) * screenWidth;
float gifY = (config.yPos / 100f) * screenHeight;

if (mouseX >= gifX && mouseX <= gifX + config.width && mouseY >= gifY && mouseY <= gifY + config.height) {
...
config.xPos = (newX / screenWidth) * 100f;
config.yPos = (newY / screenHeight) * 100f;
config.save();
}
...
float deltaX = (float) mouseX - resizeStartX;
float deltaY = (float) mouseY - resizeStartY;

config.width = Math.max(50, startWidth + deltaX);
config.height = Math.max(50, startHeight + deltaY);
config.save();
}
}